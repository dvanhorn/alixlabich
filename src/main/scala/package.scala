...
package object cek {
  ...
  @tailrec private def evalCesk(ce: Closure, s: Store, k: Kontinuation, d: Boolean = false): Value = {
    ...
    case (Closure(v: Value, e), s, Op(o, vcs, Nil, k1)) => // cesk5
      evalCesk(Closure(reduce(o, (Closure(v, e)::vcs).reverse), e), s, k1, d)
    ...
    case (Closure(Letrec((x, m)::xms, n), e), s, k) => // al3
      val l = s.next
      evalCesk(Closure(m, e.bind(x, l)), s.alloc(l), Lr(x::xms.map(_._1), Nil, xms.map(_._2), n, k), d)
    case (Closure(v: Value, e), s, Lr(x::y::xs, vvs, m::ms, n, k1)) =>
      val l = s.next
      val e1 = e.bind(y, l)
      evalCesk(Closure(m, e1), s.rebind(e(x), Closure(v, e1)).alloc(l), Lr(y::xs, (x, v)::vvs, ms, n, k1), d)
    case (Closure(v: Value, e), s, Lr(x::Nil, vvs, Nil, n, k1)) =>
      evalCesk(Closure(n, e), ((x, v)::vvs).reverse.foldRight(s)((n, a) => a.rebind(e(n._1), Closure(n._2, e))), k1, d)
